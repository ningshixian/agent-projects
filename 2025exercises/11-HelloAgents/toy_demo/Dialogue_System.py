...
llm = ChatOpenAI(
    ...
    api_key=os.getenv("LLM_API_KEY") or "",  # 添加默认值以避免None
...
def understand_query_node(state: SearchState) -> SearchState:
    ...
    conversation_history = "\n".join([
        ...
        for msg in state["messages"][-6:]  # 保留最近3轮对话
...
搜索词：[最佳搜索关键词]"""
    ...
    if "搜索词：" in response_text:
        parts = response_text.split("搜索词：")
        if len(parts) > 1:
            search_query = parts[1].strip()
    elif "搜索关键词：" in response_text:
        parts = response_text.split("搜索关键词：")
        if len(parts) > 1:
            search_query = parts[1].strip()
    
    # 状态变更
    state["messages"].append(AIMessage(content=f"我理解您的需求：{response.content}" if response.content else ""))
    state["user_query"] = str(response.content) if response.content else ""
    state["search_query"] = str(search_query)
...
def tavily_search_node(state: SearchState) -> SearchState:
    ...
    try:
        ...
        # 状态变更
        state["messages"].append(AIMessage(content=f"✅ 搜索完成！找到了相关信息，正在为您整理答案..."))
        ...
        # 状态变更
        state["messages"].append(AIMessage(content=f"❌ 搜索遇到问题，我将基于已有知识为您回答"))
        state["search_results"] = str(f"搜索失败：{error_msg}")
        state["step"] = "search_failed"
...
async def main():
    ...
    # 固定会话ID以维持多轮对话
    config = {"configurable": {"thread_id": "search-session-main"}}
    # 初始状态
